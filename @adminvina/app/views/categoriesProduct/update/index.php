<?php
  ...
  $cateProduct = $h->getById($table, $id);
  $titleCate = $cateProduct['titleCate'.$lngDefault];
    
  $imgShareFb = $cateProduct['imageShareFb'];
  if (file_exists($def['imgUploadCateProductRealPath'].$imgShareFb) && !is_null($imgShareFb) && $imgShareFb != '') {
    $imgShareFbShow = '<img src="'.$def['imgUploadCateProduct'].$imgShareFb.'" width="90" height="auto" />';
    $dImgCateFb = ' style="display: block"';
  } else {
    $imgShareFbShow = '';
    $dImgCateFb = '';
  }
  $cateID = $cateProduct['cateID'];
  $active = $cateProduct['active'];
  $sortOrder = $cateProduct['sortOrder'];
  $titleSeo = $cateProduct['titleSeo'];
  $descriptionSeo = $cateProduct['descriptionSeo'];
  $keywordSeo = $cateProduct['keywordSeo'];  
?>
<!-- select2 -->
...
<div class="modal-dialog modal-lg">
  <div class="modal-content">
    <div class="modal-header bg-success">
      <h5 class="modal-title text-uppercase"><?php echo $lang['updateCateProductText'] ?></h5>
      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true" class="text-white">&times;</span>
      </button>
    </div>
    <!--  -->
    <form method="post" action="<?php echo $def['cateProductUpdateProcess'] ?>" id="form_update" enctype="multipart/form-data">
      <div class="modal-body container-fluid">
        <div class="row">
          <input type="hidden" name="idCateProduct" value="<?php _e($id) ?>" />
          <div class="col-md-6">
            ...
            <div class="form-group">
              ...
              <div class="select2-success">
                <select class="form-control select2 select2-success" data-dropdown-css-class="select2-success" style="width: 100%;" name="cateID" id="cateID">
                  ...
                  <?php
                    $cates = $h->getAll($table, "deleted_at is null and active = 1 and cateID = 0", "id asc");                  
      ...
      <div class="modal-footer justify-content-between">
        ...
        <button type="submit" id="updateCateProduct" class="btn btn-success"><?php echo $lang['updateText'] ?> <i class="fas fa-save"></i></button>
      </div>
    </form>
  </div>
  <!-- /.modal-content -->
...
<style type="text/css">
  #display-image-sharefb-e {display: none;margin-top: 5px}
</style>
