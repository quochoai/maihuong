jQuery(document).ready(function($) {
  ...
  function fill_datatable(search_value, cateID) {
		...
		if (typeof cateID === 'undefined')
			cateID = '';
		$(table_id).DataTable({
			"aoColumnDefs": [ {"bSortable" : false, "bAutoWidth": true, "aTargets" : [3,4,5] } ],
			"paging": true,
			"lengthChange": true,
			"ordering": false,
			"info": true,
			...
			columns: [
				...
				{ data: 'subCate', name: 'subCate',className: "text-center text-nowrap small_text" },
  ...
  $('#btnReset').click(function(){
	...
	$(document).on('click', '#delete_multi', function() {
		...
		if (multi_id.length < 1)
			...
			let conf = "";
			if (multi_id.length == 1)
				conf = deleteConfirmText + cateProductText + ' ' + thisText + ' ?';
			else
				conf = deleteMultiConfirmText + cateProductText + ' ' + thisText + ' ?';
			if (confirm(conf)) {
	...
	$(document).on('click', '.delete', function() {
		...
		let conf = deleteConfirmText + cateProductText + ' ' + thisText + ' ?';
		if (confirm(conf)) {
	...
	$(document).on('click', '.active', function() {
		...
		let conf = "";
		if (act == 1)
			conf = activeConfirmText + cateProductText + ' ' + thisText + ' ? ' + effectTo + activeForm + ' ' + multiple + ' ' + productText + ' ' + theSameCate;
		else 
			conf = deactiveConfirmText + cateProductText + ' ' + thisText + ' ? ' + effectTo + deactiveForm + ' ' + multiple + ' ' + productText + ' ' + theSameCate;
		if (confirm(conf)) {
			$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>');
  ...
  $(document).on('click', '.add', function(){
	...
	$(document).on('click', addCate, function() {
		...
		if (titleCate == '') {
			toastr.error(notFill + 'tiêu đề ' + cateProductText);
	...
	let updateCate = '#updateCateProduct';
