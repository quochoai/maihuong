jQuery(document).ready(function($) {
  ...
  function fill_datatable(search_value, cateID) {
		...
		$(table_id).DataTable({
			...
			columns: [
				...
				{ data: 'titleProduct', name: 'titleProduct', className: "text-left small_text" },
				{ data: 'titleCate', name: 'titleCate', className: "text-left text-nowrap small_text" },
				{ data: 'imageProduct', name: 'imageProduct',className: "text-center text-nowrap small_text" },
				{ data: 'price', name: 'price',className: "text-center text-nowrap small_text" },
  ...
  $('#btnReset').click(function(){
	...
	$(document).on('click', '#delete_multi', function() {
		...
		if (multi_id.length < 1)
			...
			let conf = "";
			if (multi_id.length == 1)
				conf = deleteConfirmText + productText + ' ' + thisText + ' ?';
			else
				conf = deleteMultiConfirmText + productText + ' ' + thisText + ' ?';
			if (confirm(conf)) {
	...
	$(document).on('click', '.delete', function() {
		...
		let search_value = $.trim($('#search_value').val());
		let cateID = $('#filterCateID').val();
		let conf = deleteConfirmText + productText + ' ' + thisText + ' ?';
		if (confirm(conf)) {
  ...
  $(document).on('click', '.add', function(){
	...
	$(document).on('click', addProduct, function() {
		let titleProductElement = $('#titleProduct'+lngDefault);		
		let titleProduct = $.trim(titleProductElement.val());
		if (titleProduct == '') {
			toastr.error(notFill + 'tiêu đề ' + productText);
			titleProductElement.addClass('is-invalid');
			titleProductElement.focus();
			return false;
		} else {
			titleProductElement.removeClass('is-invalid');
			titleProductElement.addClass('is-valid');
		}
		let formAdd = $('#form_add');
		formAdd.ajaxForm({
			beforeSend: function() {
				btnAddProduct.attr("disabled",true);
				btnAddProduct.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> ' + processing); 
			},
			...
			complete: function(xhr) {
				btnAddProduct.html(saveText + ' <i class="fas fa-save">');
	...
	let btnUpdateProduct = $(updateProduct);
	$(document).on('click', updateProduct, function() {
		let titleProductElement_e = $('#titleProduct'+lngDefault+'_e');		
		let titleProduct_e = $.trim(titleProductElement_e.val());
		if (titleProduct_e == '') {
			toastr.error(notFill + 'tiêu đề ' + productText);
			titleProductElement_e.addClass('is-invalid');
			titleProductElement_e.focus();
			return false;
		} else {
			titleProductElement_e.removeClass('is-invalid');
			titleProductElement_e.addClass('is-valid');
		}
		let formUpdate = $('#form_update');
		formUpdate.ajaxForm({
			beforeSend: function() {
				btnUpdateProduct.attr("disabled",true);
				btnUpdateProduct.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> ' + processing); 
			},
			uploadProgress: function(event, position, total, percentComplete) {
			...
			complete: function(xhr) {
			 	btnUpdateProduct.html(updateText + ' <i class="fas fa-edit">');
