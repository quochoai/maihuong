<?php
  ...
  if (!isset($mod[1]) || $mod[1] == '') {
    $whereProduct = $where;
    $titleGroup = $lang['allProduct'];
  } elseif ($mod[2] == '' && $mod[1] != '') {
    $checkCate = $h->checkExist($tableCate, "deleted_at is null and active = 1");
    $whereProduct = '';
    if ($checkCate) {
      $cateProduct = $h->getAllSelect("id, titleCate", $tableCate, "deleted_at is null and active = 1", "id asc");
      foreach ($cateProduct as $cate) {
        $linkCate = chuoilink($cate['titleCate']);
        if ($linkCate == $mod[1]) {
          $titleGroup = $cate['titleCate'];
          $cateID = $cate['id'];
          $whereProduct = $where." and p.cateID = $cateID";
          break;
        }
      }
    }
      ...
      <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
        <div class="category-products clearfix">
          ...
          <script type="text/javascript">
            var linkData = "<?php _e(_views.'product/') ?>";
            var whereProduct = "<?php _e($whereProduct) ?>";
          </script>
          <script type="text/javascript" src="<?php _e(_views.'product/') ?>data.js"></script>
          <?php } else _e('<div class="product-grid clearfix text-center">'.$lang['temporaryNotData'].'</div>') ?>
        </div>
      </div>
